...
class NetworkMobject(VGroup):
    ...
    def spike(self, index):
        '''
        circles = []
        for edge in self.edge_groups[index]:
            circle = Circle(color = RED,radius = 0.5).move_to(edge.start)
    ...
    def activate_edges(self, index):
        self.edge_group_copy = self.edge_groups[index].copy()
        self.edge_group_copy.set_stroke(
            self.edge_propogation_color,
            width = 1.5*self.edge_stroke_width
        )
        point = Point([1,1,0])
        #print(self.edge_group_copy.get_center())
        return [FadeIn(
            self.edge_group_copy,
            run_time = self.edge_propogation_time/2,
        )]
