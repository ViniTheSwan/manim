...
from manimlib.mobject.svg.tex_mobject import TexText, Tex
...
class TextNoWrap(Text):
    def __init__(self, text, **kwargs):
        Text.__init__(self, text, size = 0.1,**kwargs)
        self = self.scale(10)
...
class Paragraph(VGroup):
    ...
    def __init__(self, *text, **config):
        ...
        for line_no in range(self.lines_list.__len__()):
            tabs = re.findall("^\\t|(?<=\\t)\\t", self.lines_list[line_no])
            self.lines_list[line_no] = self.lines_list[line_no].replace("\t", " ")
            self.lines[0].append(TextNoWrap(self.lines_list[line_no], **config))

            if tabs:
                #print(tabs)
                #print(len(tabs))
                self.lines[2].append(len(tabs))
            else:
                self.lines[2].append(0)
        self.char_height = TextNoWrap("(", **config).get_height()
    ...
    def change_alignment_for_a_line(self, alignment, line_no):
        ...
        if self.lines[1][line_no] == "center":
            ...
            self[line_no].move_to(self.get_top() +
                                  ...
                                  np.array([- self.get_width() / 2 + self.lines[0][line_no].get_width() / 2,
                                            - line_no * (self.char_height + self.line_spacing), 0])
                                  +RIGHT*self.lines[0][0]/120*self.lines[2][line_no]#######
                                  )
            ...
            if self.lines[1][line_no] == "center":
                ...
                self.lines[0][line_no].move_to(np.array([0, 0, 0]) +
                                               np.array([self.lines[0][line_no].get_width() / 2,
                                                         - line_no * (self.char_height + self.line_spacing), 0])
                                               + RIGHT * self.lines[0][0].font_size/120*self.lines[2][line_no]
                                               )
                #self.lines[0][-1].shift(3*LEFT)
